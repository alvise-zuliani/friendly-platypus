  ...
  def build(self, col_unit=1.0):
    ...
    for item in self.children:
      ...
      if isinstance(child, Row):
        built = child.build(col_unit=col_unit)
  ...
  def _add_cells_style(self, table_style):
    for index, cell in enumerate(self.children):
        if not isinstance(cell, Cell):
            continue  # Skip Columns or other direct children
        if cell.padding:
  ...
  def build(self, col_unit=1.0):
    ...
    for content in self.children:
      if isinstance(content, Cell):
        ...
        child = content  # Allow direct Column, Paragraph, etc.
      ...
      if isinstance(child, Row):
        ...
        built_child = child  # e.g., Paragraph, Spacer

      data[0].append(built_child)

      # Handle width
      if isinstance(content, Cell):
        width_spec = content.width
      elif isinstance(content, Column):
        width_spec = content.width
      else:
        width_spec = WidthUnit(12)  # Default to full width if unknown

      resolved_width = (
        width_spec.multiplier * col_unit
        if isinstance(width_spec, WidthUnit)
        else width_spec
      )
